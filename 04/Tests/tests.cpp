...
TEST(test, expressLane) {
    ...
    if (products.size() == 3) {
        EXPECT_NE(products.end(),
                  find_if(products.begin(), products.end(), [&](const Product &e) { return e.getId() == "pastaX1"; }));
        EXPECT_EQ(products.end(), find_if(products.begin(), products.end(),
                                          [&](const Product &e) { return e.getId() == "peanutsYY"; }));
        EXPECT_NE(products.end(), find_if(products.begin(), products.end(),
                                          [&](const Product &e) { return e.getId() == "cookieSA2"; }));
        EXPECT_NE(products.end(),
                  find_if(products.begin(), products.end(), [&](const Product &e) { return e.getId() == "milkAS"; }));
    }
...
TEST(test, minPlatforms){
    ...
    EXPECT_EQ(1,FunSortProblem::minPlatforms({9.00, 11.00, 12.35}, {10.00, 12.00, 12.40}));  //9a 10d 11a 12d 12.35a 12.4d
